...
pub fn restore_files(files: Vec<String>, flags: Vec<String>, config: &Config) -> Result<(), Error> {
    let write_options: WriteOptions = WriteOptions { 
        overwrite_enabled: flags.iter().any(|f| f.contains("o")),
    };
    for file in files.iter().skip(2) {
        // let (o, f): (PathBuf, PathBuf) = clear_metadata(fname)?;
...
fn restore_file(file: &String, wo: &WriteOptions, config: &Config) -> Result<(PathBuf, PathBuf), Error> { 
...
fn clear_metadata(o: &PathBuf, f: &PathBuf, c: &Config) -> Result<(), Error> {
